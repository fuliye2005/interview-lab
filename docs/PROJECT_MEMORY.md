...
- 当前批次：启动时按组件从最近 SQLite 备份自动恢复损坏的配置/材料/会话，并在诊断区提示恢复结果，待提交。
...
本批已重新跑类型检查、41 项测试、前端生产构建和 `cargo check`，结果均通过；自动恢复批次的 Tauri release 构建需在本轮提交前重新执行。
...
5. 提交并推送当前自动恢复批次，再补迁移版本、启动环境检查和 Provider 健康历史。

...
最近一次已验证结果：66 项 Vitest 通过、前端生产构建通过、`cargo check --locked` 通过；`0.2.18` Tauri EXE-only release 构建通过；SQLite v2 迁移编译通过；`0.2.8` Tauri release、NSIS 和 MSI 构建均完成；0.2.8 release EXE 已在本机启动，标题为 `Interview Lab · 实时语音测试台`，窗口可见、进程响应正常且文件版本为 `0.2.8`；Windows UI Automation 识别为 `Tauri Window` + `WRY_WEBVIEW`，未出现浏览器地址栏或标签页。版本 `0.2.9`、`0.2.10`、`0.2.11`、`0.2.12`、`0.2.13`、`0.2.14`、`0.2.15`、`0.2.16`、`0.2.17` 和 `0.2.18` 的 EXE-only release 构建均已完成；本批没有生成安装包。
...
6. 本次工作点：`0.2.18` 将配置和材料单项保存改为事务写入，并延续 `0.2.17` 的托盘悬浮入口；66 项测试、前端生产构建、`cargo check --locked` 和 EXE-only release 构建均已通过；安装包按用户要求不自动生成。每次后续发布继续递增版本号，不重复使用旧版本号。
...
本次目标审计结论：除上述运行时验收和浏览器 WebSocket 的协议边界外，规划中的配置中心、ASR Provider 中心（不含已取消的双通道/本地 ASR）、悬浮面试台、连续会话、非开发岗位回答策略、GitHub/Gitee 公共仓库材料导入、数据恢复和单文件 EXE 发布链路均已有对应实现。豆包只保留预配置、保存、脱敏预览和诊断，不再作为待实现项。

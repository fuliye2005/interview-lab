...
最近一次已验证结果：53 项 Vitest 通过、前端生产构建通过、`cargo check` 通过；SQLite v2 迁移编译通过；`0.2.4` Tauri release、NSIS 和 MSI 构建均完成。

## 当前工作树

当前分支为 `main`，本地与 `origin/main` 已包含发布提交 `d4f2e25`；当前可用版本为 `0.2.4`。最新 release EXE 已复制到 `D:\Interview Lab\tauri-app.exe`，SHA-256 为 `CC3293FDDB97FE40A261599CF1A1FEB65CCCF84C6CF4F4345FC246DCC4790F05`；桌面安装包为 `Interview Lab 0.2.4 安装包.exe` 和 `Interview Lab 0.2.4 安装包.msi`。4 张 `artifacts-window-*.png` 仅为本地诊断截图，不纳入版本库：
...
上一批已通过 47 项 Vitest、前端生产构建、`cargo check`、Tauri `--no-bundle` release 构建和 Windows 安装包构建；安全脱敏批次为 49 项测试；本轮 Provider 增强后为 53 项 Vitest、前端生产构建和 `cargo check` 均通过。`0.2.4` 的 Tauri release、NSIS 和 MSI 构建已完成，EXE 已复制到交付目录并核对哈希。
...
5. 本次交付点：`0.2.4` 已生成并推送，桌面 EXE 与 NSIS/MSI 安装包已更新；每次后续发布继续递增版本号，不重复使用旧版本号。真实数据库损坏演练和用户在 release EXE 中的悬浮窗、托盘菜单验收仍待继续。

...
- 当前批次：启动时按组件从最近 SQLite 备份自动恢复损坏的配置/材料/会话，并在诊断区提示恢复结果；模型 Provider 健康状态、启动运行环境诊断和完整快照事务写入已接入。

最近一次已验证结果：47 项 Vitest 通过、前端生产构建通过、`cargo check` 通过、Tauri `--no-bundle` release 构建通过；release EXE 标题为 `Interview Lab · 实时语音测试台`。

## 当前工作树

当前分支为 `main`，本批改动已形成独立提交，远程同步后以同一提交为准；4 张 `artifacts-window-*.png` 仅为本地诊断截图，不纳入版本库：
...
本批已重新跑 47 项测试、前端生产构建、`cargo check` 和 Tauri `--no-bundle` release 构建，结果均通过；release EXE 已更新至当前工作树。
...
5. 提交并推送当前批次；再补迁移版本/回滚演练，并由用户在 release EXE 中验收悬浮窗交互。
...
- 数据库损坏后的自动恢复已覆盖 JSON/会话组件；仍需补数据库迁移版本化和真实损坏演练。

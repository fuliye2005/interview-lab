...
- 当前批次：启动时按组件从最近 SQLite 备份自动恢复损坏的配置/材料/会话，并在诊断区提示恢复结果；模型 Provider 健康状态和启动运行环境诊断已接入。

最近一次已验证结果：46 项 Vitest 通过、前端生产构建通过、`cargo check` 通过、Tauri `--no-bundle` release 构建通过；release EXE 标题为 `Interview Lab · 实时语音测试台`。
...
本批已重新跑 46 项测试、前端生产构建、`cargo check` 和 Tauri `--no-bundle` release 构建，结果均通过；release EXE 已更新至当前工作树。

...
type SessionMode = "idle" | "asr" | "manual";
...
function App() {
  ...
  async function startSession() {
    ...
    try {
      ...
      setNotice(materials.confirmed ? "正在捕获默认系统输出并发送给 ASR。原始音频不会保存。" : "正在捕获系统音频；未确认候选人材料，将使用通用回答上下文。");
    } catch (error) {
      asr.close();
      setAsrStatus("error");
  ...
  function startManualSession() {
    ...
    setAsrStatus("idle");
    setPartial("");
    setNotice("手动测试已开始。可直接在转写结果框输入问题并生成回答。");
  }
  async function stopSession() {
    const wasAsrSession = sessionMode === "asr";
    asrRef.current?.close(); asrRef.current = undefined;
    if (wasAsrSession) await invoke("stop_system_audio_capture").catch(() => undefined);
  ...
  async function submitQuestion() {
    if (!sessionActive || sessionMode === "manual") {
  ...
  async function generateAnswer(rawQuestion: string) {
    ...
    try {
      ...
      if (sessionMode === "asr") setQuestion("");
  ...
  const statusLabel = sessionMode === "manual" && sessionActive ? "手动测试" : asrStatus === "listening" ? "ASR 正在转写" : asrStatus === "finalizing" ? "提交中" : asrStatus === "error" ? "连接异常" : "未开始";
  const statusClass = sessionMode === "manual" && sessionActive ? "manual" : asrStatus;
  ...
  return <main className="app-shell">
    ...
    <section className="content"><header className="topbar"><div><p className="eyebrow">WINDOWS · REALTIME ASR · LLM</p><h1>{({ session: "会话控制", materials: "候选人材料", settings: "服务配置", history: "文本记录" })[tab]}</h1></div><span className={`status ${statusClass}`}>{statusLabel}</span></header>
      <p className="notice">{notice}</p>
      {tab === "session" && <section className="session-grid">
        <div className="panel"><div className="panel-head"><div><h2>系统音频会话</h2><p>默认输出设备 · PCM16 / Mono / 16kHz</p></div><div className="button-row">{sessionActive ? <button className="danger" onClick={() => void stopSession()}>结束会话</button> : <><button className="primary" onClick={() => void startSession()}>开始 ASR</button><button onClick={startManualSession}>手动测试</button></>}<button disabled={!sessionActive && !question.trim()} onClick={() => void submitQuestion()}>提交当前问题</button></div></div><p className="shortcut">全局快捷键 <kbd>{settings.shortcut}</kbd> · 手动测试不需要 ASR 或候选人材料</p><label>实时增量转写</label><div className="transcript">{partial || "等待系统音频…"}</div><label>转写结果 / 当前问题（可直接输入）</label><textarea rows={6} value={question} onChange={(event) => setQuestion(event.target.value)} placeholder="ASR 最终文本会显示在这里；也可直接输入问题测试模型。" /></div>

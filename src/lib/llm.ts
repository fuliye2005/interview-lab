...
import { repositoryContextsFromMaterials } from "./repository";
...
export function buildInterviewPrompt(question: string, materials: MaterialContext, previousTurns: InterviewTurn[], detail: AnswerDetail = "balanced", focus: InterviewFocus = "technical-business", contextWindow = 8000, stageSummary = "", framework: AnswerFramework = "balanced") {
  ...
  const confirmedRepositories = repositoryContextsFromMaterials(materials).filter((repository) => repository.confirmed);
  const repositoryIndex = confirmedRepositories.map((repository, index) => `${index + 1}. ${repository.name}${repository.description ? `：${repository.description}` : ""}`).join("\n");
  const repositoryDetails = confirmedRepositories.map((repository, index) => `项目仓库 ${index + 1}：${repository.name}\n仓库摘要：\n${repository.summary.slice(0, 1400)}\n\n目录与关键配置：\n${repository.fileTree.slice(0, 1200)}\n${repository.keyFiles.slice(0, 2200)}`).join("\n\n");
  const repositoryContext = confirmedRepositories.length
    ? `已确认项目仓库数量：${confirmedRepositories.length}\n仓库索引：\n${repositoryIndex.slice(0, 12000)}\n\n项目详情（目录和关键文件为有限摘录）：\n${repositoryDetails.slice(0, 14000)}`
